...
ret+=', температура '+l.main.temp+'°C, влажность '+l.main.humidity+'%, ветер '+l.wind.speed+' м/с';var d=l.wind.deg;
if(d>337.5)ret+='⬇️';else if(d>292.5)ret+='↘️';else if(d>247.5)ret+='➡️';else if(d>202.5)ret+='↗️';
else if(d>157.5)ret+='⬆️';else if(d>122.5)ret+='↖️';else if(d>67.5)ret+='⬅️';else if(d>22.5)ret+='↙️';else ret+='⬇️';
ret+=', облачность '+l.clouds.all+'%.'}callback(err,ret)})}
...
if(cmd=='1'){
return getWeather(0,function(err,ret){ctx.reply('Сейчас '+ret);getWeather(1,function(err,ret){ctx.reply('В ближайшие три часа ожидается '+ret)});});
}
